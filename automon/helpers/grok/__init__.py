...
class Grok:
    def __init__(self):
        ...
        for folder, folders, files in walk:
            if 'legacy' in folder:
                self.legacy = {f: self.file_to_dict(os.path.join(folder, f)) for f in files}

            if 'ecs-v1' in folder:
                self.ecs_v1 = {f: self.file_to_dict(os.path.join(folder, f)) for f in files}

        self._log = Logging(name=Grok.__name__, level=Logging.DEBUG)

    @staticmethod
    def file_to_dict(file: str):
        """convert logstash grok pattern file into dict
